...
void addSH(vector<product*> &catalog){
    ...
    for (int i = 0; i < catalog.size(); i++) {
            if(catalog[i]->ID == ID) {
                ...
                break;  // Exit the loop since we've found the product
            }
        }

        // If index is still -1, the product wasn't found
        if (index == -1) {
...
void listCatalog(vector<product*> &catalog){
    for (int i = 0; i < catalog.size() ; i++){
        ...
        for (int ii = 0; ii < 10; ii++) { // Assuming 10 is the max history size
			if (catalog[i]->history[ii].quarter != "NONE") {
				hasHistory = true;
				cout << setw(5) << "Product " << i + 1 << " history" << endl;
				cout << setw(7.5) << "Quarter: " << catalog[i]->history[ii].quarter << endl;
				cout << setw(7.5) << "Amount sold: " << catalog[i]->history[ii].sales << endl;
				cout << "\n";
			}
		}
