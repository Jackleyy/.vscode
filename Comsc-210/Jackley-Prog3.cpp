/*************************************************************************************
Description: Simulates a queueing system   
...
/*
 * Queueing Simulation Starter File
 * --------------------------------
...
void runSimulation(int simTime, double arrivalRate,
    ...
    for (int curTime = 0; curTime < simTime; curTime++) {
        ...
        if (customerArrived(arrivalRate)) {
            ...
            if(customerQueue.size() == PARAMS_MAX){
                totalDropped++;
            }
            else{
        ...
        //gonna check if their current time is equal to the max service time of 15
        //if not, then we can keep looping as someone is being serviced
        //if it is equal then we set it to zero and pop the top to allow it to track the next customer
        if(curCustomerTime != serviceTime){
            curCustomerTime++;
        }
        else{
            curCustomerTime = 0;
            ...
            if (customerQueue.size() != 0) {
                ...
                customerQueue.pop();
                nServed++;//going to count served as people at how many got to the register
            }
